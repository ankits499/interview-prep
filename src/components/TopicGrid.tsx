...
export function TopicGrid() {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">
      {TOPICS.map((topic) => {
        const stats = getProgressStats(getConceptsForTopic(topic.id), getQuestionsForTopic(topic.id), isReviewed, progress)
          ...
          >
            <div className="flex items-start justify-between gap-2 sm:gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-xs text-ink-muted sm:text-sm">{topic.description}</p>
              </div>
              {stats.total > 0 && (
                <div className="relative shrink-0">
                  <ProgressRing percent={stats.percent} size={30} strokeWidth={2.5} className="sm:hidden" />
                  <ProgressRing percent={stats.percent} size={38} strokeWidth={3} className="hidden sm:block" />
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-x-2 gap-y-1 font-mono text-[10px] text-ink-muted sm:mt-4 sm:text-xs">
              ...
              <ArrowRight size={14} className="shrink-0 text-ink-muted transition-transform group-hover:translate-x-0.5 group-hover:text-accent" />
            </div>
          </Link>
        )
      })}
